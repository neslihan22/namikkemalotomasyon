    ...
    public partial class Faculty : Form
    {
        ...
        private void GetDeptId()
        {
            try
            {
                ...
                data.Columns.Add("BolumId", typeof(int));
                data.Load(Reader);
                F_DeptIdCb.ValueMember = "BolumId";
        ...
        private void GetDeptName()
        {
            ...
            {
                Connection.Open();
                string Query = "Select * from BolumTbl where BolumId = " + F_DeptIdCb.SelectedValue.ToString() + " ";
                ...
                {
                    F_DeptTb.Text = dataRow["İsim"].ToString();
                }
                Connection.Close();
            }
            catch (Exception e)
        ...
        private void Display()
        {
            try
            {
                DataTable data = new DataTable();
                Connection.Open();
                SqlDataAdapter adapter = new SqlDataAdapter("select * from FakulteTbl", Connection);
                adapter.Fill(data);
        ...
        private void SaveBtn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    Connection.Open();
                    SqlCommand cmd = new SqlCommand("Insert into FakulteTbl(İsim,İsebaslamatarihi,Cinsiyet,Adres,Nitelikler,Deneyimler,BolumId,Bolum,Maas)values(@FN,@FDOB,@FGen,@FAdd,@FQ,@FE,@FDeptId,@FDept,@FSal)", Connection);
        ...
        private void EditBtn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    Connection.Open();
                    SqlCommand cmd = new SqlCommand("Update FakulteTbl Set İsim=@FN,İsebaslamatarihi=@FDOB,Cinsiyet=@FGen,Adres=@FAdd,Nitelikler=@FQ,Deneyimler=@FE,BolumId=@FDeptId,Bolum=@FDept,Maas=@FSal where F_Id=@FKey", Connection);
                    cmd.Parameters.AddWithValue("@FN", F_NameTb.Text);
                    cmd.Parameters.AddWithValue("@FDOB", F_DOBdt.Value.Date);
                    cmd.Parameters.AddWithValue("@FGen", F_GenderCb.SelectedItem.ToString());
                    cmd.Parameters.AddWithValue("@FAdd", F_AddressTb.Text);
        ...
        private void DeleteBtn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    Connection.Open();
                    SqlCommand cmd = new SqlCommand("Delete from FakulteTbl where F_Id=@FKey", Connection);
                    cmd.Parameters.AddWithValue("@FKey", Key);
                    cmd.ExecuteNonQuery();
